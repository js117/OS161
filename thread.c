...
{
  struct thread *thread = alloc_kpages((sizeof(struct thread)+PAGE_SIZE)/PAGE_SIZE);
	if (thread==NULL) {
		return NULL;
	}
	strcpy(thread->t_name, name); //better be less than 64 chars
	...
	// them here.
	int i; //see thread.h for maxNumChildren
	for (i = 0; i < 255; i++) {
	  thread->children[i] = -1; 
	}
	...
	int x = array_add(THREAD_LIST,thread);
	assert(x==0);
	numProcs++; //will keep counting UP
...
thread_destroy(struct thread *thread)
{
	...
	if (thread->t_stack) {
	  free_kpages((void*)thread->t_stack);
	}
	...
	//PROCESS_LIST[thread->pid] = 0; 

	
	//kfree(thread->t_name);
	free_kpages((void*)thread);
...
thread_bootstrap(void)
{
	...
	process_lock = lock_create("processLock"); 
	
	THREAD_LIST = array_create();
	if (THREAD_LIST==NULL) {
	  panic("Cannot create thread list\n");
	}
	 ...
	 */
	me = thread_create("<boot/menu>"); //PID 0, don't mess with me
	if (me==NULL) {
	...
	result = array_preallocate(THREAD_LIST, 240); 
	if (result) {
	  panic("THREAD_LIST: cannot preallocate memory"); 
	}

	result = array_preallocate(sleepers, 240);
	if (result) {
	  panic("SLEEPERS: cannot preallocate memory");
	}
	result = array_preallocate(zombies, 240);
	if (result) {
	  panic("ZOMBIES: cannot preallocate memory"); 
	}
	...
	PROCESS_LIST = kmalloc(sizeof(struct addrspace *) * 240); //240: max number of processes 
	bzero(PROCESS_LIST,240);
	PROCESS_LIST[curthread->pid] = curthread->t_vmspace;
	kprintf("PROCESS_LIST initialized with: 0x%x @[%d]\n",
		PROCESS_LIST[curthread->pid],curthread->pid);
	

	/* Number of threads starts at 1 */
	numthreads = 1;
	numProcs = 0; //proc spot 0 is taken 

	maxNumChildren = 255; //max number of children per process
	    ...
	    struct thread **ret)
{
	...
	if (!strcmp("NEW_CHILD",name)) { 
	  newguy->pid = numProcs;
	  newguy->ppid = curthread->pid; 
	} 
	else {
	  newguy->pid = curthread->pid;
	  newguy->ppid = curthread->ppid;
	}

	/* Allocate a stack */
	newguy->t_stack = alloc_kpages(1); //CHANGES FROM KMALLOC
	...
	//NOTE - instead of doing this dynamically we're picking a starting number for 
	//preallocation and doing it in thread_bootstrap.
	//Scheduler preallocation moved to its own bootstrap. 
...
mi_switch(threadstate_t nextstate)
{
  ...
  //kprintf("MI_SWITCH CALLED!\n"); 
	...
	if (curthread != NULL && curthread->t_stack != NULL) {
		 ...
		 */
	  if (curthread->t_stack[0]!=(char)0xae || curthread->t_stack[1]!=(char)0x11 || 
	      curthread->t_stack[2]!=(char)0xda || curthread->t_stack[3]!=(char)0x33) {
	    process_debug();
	    examine_bitmap1();
	    examine_swapmap();
	...
	if (curthread->t_vmspace && prevPid != curPid) {
		as_activate(curthread->t_vmspace);
	}
	
	//if(curPid != prevPid) kprintf("\n\nCONTEXT SWITCH HAPPEND: old:%d --> new:%d\n\n",prevPid,curPid);
	//else { kprintf("\n\nSwitched to same process: %d --> %d\n\n",prevPid,curPid); }
...
thread_exit(void)
{
	...
	/*
	if (!strcmp(curthread->t_name,"testbin/forktest")) {
	  kprintf("\n\nTESTBIN/FORKTEST HAS LEFT THE BUILDING\n"); 
	  kprintf("testbin/forktest: 0x%x\n",curthread);
	  kprintf("as->page_table: 0x%x\n",as->page_table); 
	  kprintf("as->page_table_info: 0x%x\n",as->page_table_info); 
	}
	*/
	
	//V(procSem); 
	//there better not be anybody sleeping on us...
	thread_wakeup(curthread); 
	//lock_acquire(forkLock);
	
	//lock_release(forkLock); 

	int i = array_getindex(THREAD_LIST,curthread);
	array_remove(THREAD_LIST,i); 
...
thread_sleep(const void *addr)
{
	...
	//kprintf("THREAD TO SLEEP ON ADDR: %x\n", addr); 
	
	curthread->t_sleepaddr = addr;
	curthread->isSleeping = 1; 
	mi_switch(S_SLEEP);
...
//thread_wakeup_single will only wake up a single thread sleeping on
//address "addr". Note that the usual thread_wakeup goes through the entire
//sleepers array to wakeup every single thread; in our case we simply break
//once we have woken up a thread. 
void
thread_wakeup_single(const void *addr)
{
	...
	for (i=0; i<array_getnum(sleepers); i++) {
		...
		if (t->t_sleepaddr == addr) {
			...
			result = make_runnable(t);
			assert(result==0);
			break; //we've woken up a single thread; job done
		}
	}
}
